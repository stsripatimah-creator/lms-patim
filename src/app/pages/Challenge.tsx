...
export function Challenge() {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedOption === QUESTION.correct) {
      ...
      toast.success(`Jawaban Benar! +${XP_REWARD} XP`);
    } else {
      setAnswered(false);
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="text-center space-y-2">
          <Badge variant="outline" className="border-orange-500/50 text-orange-500 bg-orange-500/10 px-3 py-1">
            <Zap className="h-3 w-3 mr-1" /> Daily Streak: {streak} Hari
          </Badge>
          <h1 className="text-3xl font-bold">Challenge Hari Ini</h1>
          ...
          <CardFooter className="pt-2 pb-6">
            {answered === true ? (
              <Button className="bg-green-600 hover:bg-green-700 w-full" onClick={handleKlaimReward}>
                Klaim Reward (+{xpToday || XP_REWARD} XP)
              </Button>
            ) : (
        ...
        <Card className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border-indigo-500/30 overflow-hidden relative">
          ...
          <CardHeader>
            ...
            <CardTitle className="mt-4 text-xl flex items-center gap-2">
              <Code className="h-5 w-5 text-indigo-400" /> Coding Challenge
            </CardTitle>
            <p className="text-slate-400 mt-2">Membuat Card Produk Responsif dengan HTML & CSS.</p>
          </CardHeader>
          <CardContent>
            <div className="space-y-2 mb-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Kesulitan</span>
                <Badge variant="outline" className="text-indigo-400 border-indigo-500/50">Menengah</Badge>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-400">Estimasi Waktu</span>
                <span className="text-white font-medium">15 menit</span>
              </div>
            </div>
          </CardContent>
          <CardFooter className="pt-2 pb-6">
            <Button className="bg-indigo-600 hover:bg-indigo-700 w-full group" onClick={() => toast.info("Coding challenge segera hadir!")}>
              Mulai Challenge <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </Button>
          </CardFooter>
        </Card>
        ...
        {answered === false && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p><strong>Hint:</strong> Properti ini bekerja pada cross-axis. Jika flex-direction adalah row (default), maka cross-axis adalah vertikal.</p>
          </motion.div>
        )}
      ...
      <div className="space-y-6">
        <Card className="bg-gradient-to-br from-orange-500/10 to-red-500/10 border-orange-500/30">
          ...
          <CardContent>
            ...
            <p className="text-sm text-slate-400">Target: 200 XP/hari</p>
        ...
        <Card className="bg-[#1E293B] border-slate-700">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
              <span className="text-sm text-slate-400">XP Bonus</span>
              <span className={`font-bold ${xpBonus > 0 ? 'text-green-500' : 'text-slate-500'}`}>+{xpBonus}%</span>
            </div>
            <p className="text-xs text-slate-500 mt-2">Pertahankan streak untuk bonus XP lebih besar!</p>
          </CardContent>
        </Card>

        <Card className="bg-[#1E293B] border-slate-700">
          <CardHeader><CardTitle className="text-base">Badge Selanjutnya</CardTitle></CardHeader>
          <CardContent>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">30 Challenges</p>
              </div>
            </div>
