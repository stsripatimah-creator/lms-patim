...
export function LessonPlayer() {
  ...
  const navLessons = (NAV_DATA[trackId as keyof typeof NAV_DATA] || []).map(nav => ({
    ...nav,
    ...
    if (user && lessonId) {
      try {
        ...
        if (profileData) {
          ...
          if (lastActive === today) {
            // Sudah belajar hari ini, streak tidak berubah
          ...
          // Update points + streak + last_active
    ...
    <div className={cn(
      ...
      {showCertificate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-[#1E293B] border border-slate-700 rounded-2xl p-8 max-w-lg w-full text-center shadow-2xl">
            <div className="text-6xl mb-4">🎓</div>
            <h2 className="text-2xl font-bold text-white mb-2">Selamat!</h2>
            <p className="text-slate-400 mb-6">
              Kamu telah menyelesaikan seluruh materi <span className="text-[#4F46E5] font-bold">{lesson.track}</span>!
            </p>

            {/* Certificate Preview */}
            <div
              id="certificate-content"
              className="bg-gradient-to-br from-[#4F46E5]/20 to-purple-600/20 border-2 border-[#4F46E5]/50 rounded-xl p-6 mb-6"
            >
              <div className="text-xs text-slate-400 uppercase tracking-widest mb-2">Sertifikat Penyelesaian</div>
              <div className="text-lg font-bold text-white mb-1">{user?.email?.split("@")[0] || "Pengguna"}</div>
              <div className="text-slate-400 text-sm mb-3">Telah berhasil menyelesaikan</div>
              <div className="text-xl font-bold text-[#4F46E5]">{lesson.track} Fundamentals</div>
              <div className="text-slate-500 text-xs mt-3">StepByWeb • {new Date().toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" })}</div>
            </div>
              ...
              <Button
                onClick={() => {
                  ...
                  printWindow?.document.write(`
                    ...
                    <style>body{font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#0B1220;color:white;}
                    .cert{background:linear-gradient(135deg,rgba(79,70,229,0.3),rgba(124,58,237,0.3));border:2px solid #4F46E5;border-radius:16px;padding:48px;text-align:center;max-width:500px;}
                    .label{font-size:12px;letter-spacing:4px;color:#94a3b8;text-transform:uppercase;margin-bottom:8px;}
                    .name{font-size:24px;font-weight:bold;margin-bottom:8px;}
                    .course{font-size:28px;font-weight:bold;color:#4F46E5;margin-top:12px;}
                    .date{font-size:12px;color:#64748b;margin-top:16px;}
                    </style></head>
                    <body><div class="cert">
                    <div class="label">Sertifikat Penyelesaian</div>
                    <div class="name">${user?.email?.split("@")[0] || "Pengguna"}</div>
                    <div style="color:#94a3b8;font-size:14px;">Telah berhasil menyelesaikan</div>
                    <div class="course">${lesson.track} Fundamentals</div>
                    <div class="date">StepByWeb • ${new Date().toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" })}</div>
                    </div></body></html>
                  `);
