...
import { ArrowLeft, ChevronRight, CheckCircle, Save, Code, Monitor, ChevronDown, RefreshCw, Play } from "lucide-react";
...
export function CourseDetail() {
  ...
  const handleSave = () => notify.success("Progress tersimpan");
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] -m-4 md:-m-6 lg:-m-8">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 bg-[#0B1220] px-4 py-3 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" onClick={handleSave} className="hidden sm:flex">
            <Save className="mr-2 h-4 w-4" /> {t('courses.save')}
          </Button>
          <Button onClick={() => notify.success("Marked as completed!")} className="bg-green-600 hover:bg-green-700 text-white">
            <CheckCircle className="mr-2 h-4 w-4" /> {t('courses.finish')}
          </Button>
        </div>
      </div>
              ...
              {[1, 2, 3].map((module) => (
                <div key={module} className="border-b border-slate-800/50">
                  <div className="flex items-center justify-between p-3 bg-slate-800/30 text-xs font-bold text-slate-400">
                    <span>{t('courses.module')} {module}</span>
          ...
          <Panel defaultSize={40} minSize={30} className="flex flex-col border-r border-slate-800 bg-[#0B1220]">
            ...
            <div className="p-4 border-t border-slate-800 flex justify-between bg-[#0B1220]">
              <Button variant="outline" className="border-slate-700 hover:bg-slate-800 text-slate-300">
                <ArrowLeft className="mr-2 h-4 w-4" /> {t('courses.previous')}
              </Button>
              <Button className="bg-[#4F46E5] hover:bg-[#4338ca] text-white">
                {t('courses.next')} <ChevronRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
          </Panel>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
              <div className="flex items-center justify-between border-b border-slate-800 bg-[#1E1E1E] px-4 py-2 shrink-0">
                <TabsList className="bg-slate-800 h-8">
                  <TabsTrigger value="editor" className="text-xs h-7 data-[state=active]:bg-[#2e2e2e] data-[state=active]:text-white">
                    <Code className="mr-2 h-3 w-3" /> Editor
                  </TabsTrigger>
                  <TabsTrigger value="output" className="text-xs h-7 data-[state=active]:bg-[#2e2e2e] data-[state=active]:text-white">
                    <Monitor className="mr-2 h-3 w-3" /> Output
                  </TabsTrigger>
                </TabsList>
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" onClick={handleRun} disabled={isRunning} className="bg-[#22C55E] hover:bg-green-600 text-white h-8 text-xs px-3">
                    <Play className="mr-1.5 h-3 w-3 fill-current" /> {isRunning ? t('courses.running') : t('courses.run')}
                  </Button>
                </div>
              </div>
                  ...
                  <div className="w-full h-full p-4 font-mono text-sm text-green-400">
                    {output
                      ...
                      : <div className="text-slate-500 italic flex flex-col items-center justify-center h-full gap-2">
                          <PlayCircleIcon className="h-8 w-8 opacity-50" />
                          {t('courses.clickRun')}
                        </div>
                    }
                  </div>
